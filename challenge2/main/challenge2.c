...
#define BUZZER_PIN 19 // กำหนดขา GPIO ที่ต่อ Buzzer (เปลี่ยนได้ตามวงจร)
#define LDR_THRESHOLD 1000 // เกณฑ์แสงน้อย
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "ระบบเตือนแสงน้อยด้วย Buzzer และควบคุม LED อัตโนมัติด้วย LDR");
    ESP_LOGI(TAG, "LDR: GPIO34 (ADC1_CH6), LED: GPIO%d (PWM), BUZZER: GPIO%d", LED_PIN, BUZZER_PIN);
    ledc_init();

    // ตั้งค่า GPIO สำหรับ Buzzer
    ...
    gpio_config(&io_conf);
    gpio_set_level(BUZZER_PIN, 0); // ปิดเสียงเริ่มต้น
    ...
    while (1) {
        ...
        // ตรวจสอบแสงน้อย ถ้าน้อยกว่าเกณฑ์ ให้ Buzzer ดังเตือน
        if (adc_reading < LDR_THRESHOLD) {
            gpio_set_level(BUZZER_PIN, 1); // เปิดเสียง
        } else {
            gpio_set_level(BUZZER_PIN, 0); // ปิดเสียง
        }
        ...
        ESP_LOGI(TAG, "ADC: %d | V: %.2f | Light: %.1f%% | LED PWM | Buzzer: %s", adc_reading, voltage, lightLevel, (adc_reading < LDR_THRESHOLD) ? "ON" : "OFF");
